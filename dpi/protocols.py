...
def is_rtp(payload: bytes) -> bool:
    ...
    Этим же признаком отличается RTCP — он ходит по тому же порту и тоже
    начинается с версии 2. Значит одна проверка закрывает оба.
    """
...
def is_voice_discovery(payload: bytes) -> bool:
    ...
    часть потока, и мусор посреди разговора рвёт сессию.

    Опознаём НЕ по точному виду заголовка, а от обратного: нужная длина и
    «это заведомо не RTP». Причина в том, что формат менялся. Нынешний
    начинается с типа 0x0001, а прежний — сразу с SSRC, и завязываться на
    первые два байта значит промахнуться мимо старых клиентов и не прикрыть
    их вовсе. Зато признак RTP надёжен и не меняется: версия 2 в старших
    двух битах первого байта, то есть 0x80..0xBF. Всё, что этой длины и не
    оттуда, — служебное, и подделка ему не повредит.
    """
    if not (VOICE_DISCOVERY_MIN <= len(payload) <= VOICE_DISCOVERY_MAX):
        return False
    return not is_rtp(payload)
