...
GROUPS["discord"] = {
    ...
    # Диапазон официальный: 50000-65535. Узкие 50000-50100 брались из
    # опасения задеть игры -- и мимо них проходила ровно демонстрация экрана,
    # которая берёт порт откуда угодно из полного диапазона. Игры при этом
    # не страдают: движок сверяет АДРЕС сервера со списком известных адресов
    # Discord и чужой трафик пропускает нетронутым (см. engine._handle_udp).
    "udp": [(19294, 19344), (50000, 65535)],
}
