...
GROUPS["discord"] = {
    ...
    # Голосовые каналы и демонстрация экрана ходят обычным UDP (не QUIC).
    #
    # Диапазон узкий, и это осознанно. В 1.2.0 он был расширен до 50000-65535
    # из догадки, что демонстрация экрана берёт порт откуда угодно, -- догадка
    # неверная: Go Live идёт по ТОМУ ЖЕ сокету к тому же голосовому серверу,
    # что и звук, просто другим потоком RTP. Порт у них один.
    #
    # Цена ошибки оказалась высокой. Полный диапазон означает, что каждый
    # исходящий UDP-пакет любой программы -- игры, торрента, чего угодно --
    # уезжает из ядра в Python и обратно. Реальное время такого не переживает:
    # голос переставал подключаться вовсе.
