...
# Какой дата-центр живёт в какой подсети — по нему выбирается имя kwsN.
...
def is_telegram_ip(ip: str) -> bool:
    try:
        ...
        return False
    return any(addr in ipaddress.ip_network(n) for n in TG_NETS)
...
class TgBridge:
    ...
    def _socks_handshake(self, client) -> Optional[Tuple[str, int]]:
        ...
        if not is_telegram_ip(ip):
            ...
            # WebSocket дата-центра всё равно не пройдёт, поэтому честный отказ
            self._reply(client, 0x02)
            return None
        return ip, port
