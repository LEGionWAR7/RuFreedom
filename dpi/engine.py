...
class Engine:
    ...
    def _build_filter(self) -> str:
        ...
        for lo, hi, _gid in self._udp_spans:
            ...
            # был как раз пакет определения адреса, и всё работало. А если в
            # канале уже кто-то есть, поток RTP начинается сразу — и подделка
            # попадала внутрь разговора, который сервер разбирает пакет за
            # пакетом. Отсюда и «в пустой канал заходит, в занятый нет».
            udp_parts.append(
    ...
    def _handle_voice(self, pkt, payload: bytes, group: str) -> None:
        ...
        шесть повторов, содержимое — полноразмерный QUIC Initial. Отличие
        одно и оно намеренное: подделка ставится не «в первые два пакета
        потока», а строго перед пакетом ОПРЕДЕЛЕНИЯ АДРЕСА. Живой разговор
        не трогается вовсе — см. пояснение в _build_filter.
        ...
        """
        # Второй рубеж, на случай если пакет всё же долетел сюда: подделку
        # ставим ТОЛЬКО перед определением адреса. Живой разговор не трогаем
        # ни при каких обстоятельствах.
        if not protocols.is_voice_discovery(payload):
            self._w.send(pkt)
            return
